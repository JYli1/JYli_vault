...
phar文件包含用gzip绕过时是不需要伪协议的，因为那个是include的底层会对phar文件进行先解压再包含，但是要注意的是php代码要写在stub才会执行。然后小技巧就是`文件中出现了.phar即可认为是phar文件`，注意有一个概念和这里有点像，

phar反序列化时说认定phar文件是靠stub头，那个说的时phar伪协议包含的
