...
![500](assets/phpGC回收机制的利用/file-20251216151421929.png)
...
![800](assets/phpGC回收机制的利用/file-20251216151502645.png)

这样操作之后就提前销毁了对象，触发了析构函数。但是这题我在本地打通了，但是在题目中没打通

在**php** **8.0之后可能是****垃圾回收****机制改变，不需要在最后将****`i:1`****修改为****`i:0`

2. 把反序列化字符串最后的大括号去掉，导致fast desturct 原理大概就是导致反序列化失败，提前触发对象销毁。